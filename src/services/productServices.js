...
export const getProductIngredients = async (productId) => {
  try {
    ...
    console.error(`❌ Failed to fetch product ingredients ${productId}:`, error);
    return [];
  }
};
...
export const getProductImages = async (productId) => {
  try {
    ...
    console.error(`❌ Failed to fetch product images ${productId}:`, error);
    return [];
  }
};

export const addProductImage = async (productId, imageUrl, altText = '', displayOrder = 0, isPrimary = false) => {
  try {
    ...
    handleError(error, 'Failed to add product image');
  }
};
...
export const getProductBundleItems = async (productId) => {
  try {
    const res = await api.get(`/products/${productId}/bundle-items`);
    return res.data;
  } catch (error) {
    console.error(`❌ Failed to fetch bundle items ${productId}:`, error);
    return [];
  }
};

export const updateProductBundleItems = async (productId, bundleItems) => {
  try {
    const res = await api.put(`/products/${productId}/bundle-items`, {
      bundle_items: bundleItems
    });
    return res.data;
  } catch (error) {
    handleError(error, 'Failed to update bundle items');
  }
};

/* =======================
   PRODUCT INGREDIENTS RELATION
======================= */

export const updateProductIngredients = async (productId, ingredientIds) => {
  try {
    const res = await api.put(`/products/${productId}/ingredients`, {
      ingredient_ids: ingredientIds
    });
    return res.data;
  } catch (error) {
    handleError(error, 'Failed to update product ingredients');
  }
};
