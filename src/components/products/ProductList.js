...
export default function ProductList() {
  ...
  return <><HubSpotListing title="Products" createLabel="Add product" rows={products} initialFields={fields} onCreate={() => { setEditing(null); setDialogOpen(true); }} onRowOpen={(row) => navigate(`/products/${row.id}`)} onUpdateRow={edit} renderValue={(field, value) => field === 'selling_price' ? `₹${Number(value || 0).toFixed(2)}` : (value ?? '—')} /><AddProductDialog open={dialogOpen} onClose={closeDialog} onAddProduct={save} productToEdit={editing} /></>;
}
