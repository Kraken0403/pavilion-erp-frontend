...
export default function HubSpotListing({
  ...
  onUpdateRow, renderValue, onRefresh, createIcon,
}) {
  const { settings } = useSettings();
  const currencyLabel = displayCurrency(settings?.currency_code || 'INR');
  const storageKey = `pav-erp:list:${String(title).toLowerCase().replace(/\s+/g, '-')}`;
  ...
  const displayValue = (field, value, row) => renderValue
    ? renderValue(field, value, row)
    : fieldKind(field, rows) === 'date' && value ? formatDate(value) : textValue(value);
